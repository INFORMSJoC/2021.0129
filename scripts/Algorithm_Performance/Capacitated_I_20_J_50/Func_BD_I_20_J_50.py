...
def direct_RMP(I, J, K, L, M_k, A_k, b_k, Sigma_k, d_bar_k,
               d_uk, d_lk, sigma_k, nu_hat, lambda_hat,
               f, p, q, r):
    print("*" * 60)
    print("Solving Relaxed Master Problem start")

    DRMP = gp.Model("Direct Relaxed Master Problem")

    y = DRMP.addVars(I, vtype=GRB.BINARY, name="y")

    alpha = DRMP.addVars(K, lb=-GRB.INFINITY, ub=+GRB.INFINITY,
                         vtype=GRB.CONTINUOUS, name="alpha")

    beta_u = DRMP.addVars(J, K, lb=0, ub=+GRB.INFINITY,
                          vtype=GRB.CONTINUOUS, name="beta_u")

    beta_l = DRMP.addVars(J, K, lb=0, ub=+GRB.INFINITY,
                          vtype=GRB.CONTINUOUS, name="beta_l")

    rho = DRMP.addVars(K, lb=0, ub=+GRB.INFINITY,
                       vtype=GRB.CONTINUOUS, name="rho")

    xi = DRMP.addVars(M_k, L, K, lb=0, ub=+GRB.INFINITY,
                      vtype=GRB.CONTINUOUS, name="xi")

    phi = DRMP.addVars(J, L, K, lb=-GRB.INFINITY, ub=+GRB.INFINITY,
    ...
    for k in range(K):
        ...
        for e_k in range(L):
            ...
            # term 4
            term_4_coef = np.multiply(q, lambda_hat[:, e_k])
            c1.add(gp.LinExpr(term_4_coef, y.select("*")), 1)

            DRMP.addConstr(c1 >= 0, "c1_{}_{}".format(k, e_k))
    ...
    for k in range(K):
        for e_k in range(L):
            for j in range(J):
                ...
                # term 2
                c2.add(gp.LinExpr(Sigma_k[:, j, k], phi.select("*", e_k, k)),
                       -1)
                # term 3 & 4
                ...
                DRMP.addConstr(c2 >= 0, "c2_{}_{}_{}".format(k, e_k, j))
    ...
    for k in range(K):
        for e_k in range(L):
            ...
            DRMP.addConstr(c3 <= 0, "c3_{}_{}".format(k, e_k))

    # objective
    obj = gp.LinExpr(f, y.select("*"))
    for k in range(K):
    ...
    DRMP.setObjective(obj, sense=GRB.MINIMIZE)

    DRMP.setParam("OutputFlag", 0)
    DRMP.setParam("TimeLimit", 18000)

    DRMP.optimize()
    run_time_DRMP = DRMP.getAttr('Runtime')
    obj_val = DRMP.getAttr("ObjVal")
    y_star = DRMP.getAttr("X", y)
    y_star_temp = np.array(y_star.select("*"))
    y_temp = np.zeros(I)
    for i in range(I):
        y_temp[i] = round(y_star_temp[i])
    alpha_star = DRMP.getAttr("X", alpha)

    beta_u_star = DRMP.getAttr("X", beta_u)

    beta_l_star = DRMP.getAttr("X", beta_l)

    rho_star = DRMP.getAttr("X", rho)

    print("Solving Relaxed Master Problem end")
    print("optimal_solution: {}".format(y_temp))
    print("optimal_value: {}".format(obj_val))
    print("computation time: {}".format(run_time_DRMP))
    print("*" * 60)
    if DRMP.status == 9:
        print("Out of Time 5h")
    return obj_val, y_star, alpha_star, beta_u_star, beta_l_star, rho_star, run_time_DRMP, 1
...
def BD_I20_J50(I, J, K, M_k, A_k, b_k, Sigma_k, d_bar_k, d_uk, d_lk, sigma_k, c, f, p, q, r, file_nameg):
    ...
    while (flag == 1 and run_time_all_temp < 18000 and LB_temp + 0.5 < UB_temp):
        ...
        print()
        print("=" * 60)
        print("           BD_Capacitated_I_{}_J_{}_K_{}:the {}th iteration".format(I, J, K, times))

        obj_val_dec, y_star_dec, alpha_star_dec, beta_u_star_dec, beta_l_star_dec, rho_star_dec, \
        run_time_dec_RMP, cycle_index = \
            direct_RMP(I, J, K, L, M_k, A_k, b_k, Sigma_k, d_bar_k,
                        d_uk, d_lk, sigma_k, nu_hat, lambda_hat,
                        f, p, q, r)
        ...
        write_xlsx(file_nameg + 'Result_BD_I_' + str(I) + '_J_' + str(J) + '_K_' + str(K) + '.xlsx',
