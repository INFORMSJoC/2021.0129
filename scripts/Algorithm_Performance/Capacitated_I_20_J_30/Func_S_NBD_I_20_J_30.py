...
def write_xlsx(file_name, data):
    """
        功能：将字典写入.xlsx文件
        输入只能为 dict {}
    :param file_name: 文件夹名\\表格名
    :param sheet_name: 工作薄名
    :return: 写入.xlsx文件
    """
    sheet_name = list(data.keys())
    workbook = xlsxwriter.Workbook(file_name)  # 建立一个表格
    for s in range(len(sheet_name)):
        worksheet = workbook.add_worksheet(sheet_name[s])  # 建立工作薄
        data_temp = np.array(data[sheet_name[s]])
        if len(data_temp.shape) == 0:  # 只有一个数
            worksheet.write(0, 0, data_temp)
        elif len(data_temp.shape) == 1:  # 只有一维的情况
            row = data_temp.shape[0]
            for i in range(row):
                worksheet.write(i, 0, data_temp[i])
        else:  # 二维的情况
            row = data_temp.shape[0]  # 读取行
            col = data_temp.shape[1]  # 读取列
            for i in range(row):
                for j in range(col):
                    worksheet.write(i, j, data_temp[i, j])  # 循环写入
...
def read_xlsx(file_name):
    """
        功能：读取xlsx表格，输出 dict{}
        只能读取数字，不能读取字符
    :param file_name: 文件夹名\\表格名称
    :return: 表格内容(dict)
    """
    wb = load_workbook(file_name)  # 导入一个表格
    sheet_name = wb.sheetnames  # 读取sheet名
    # 删除sheet1
    if "Sheet1" in sheet_name:
        sheet_name.remove("Sheet1")

    # 读取每个sheet中的内容,并用字典记录
    Data = {}
...
def S_NBD_I20_J30(I, J, K, M_k, A_k, b_k, Sigma_k, d_bar_k, d_uk, d_lk, sigma_k, c, f, p, q, r, file_nameg):
    ...
    while (flag == 1 and UB_temp > (LB_temp + 0.5)):
        ...
        print()
        print("=" * 60)
        print("              S_NBD_Capacitated_I_{}_J_{}_K_{}:the {}th interation".format(I, J, K, times))
