...
def SP_MICP(I, J, K, M_k, y_star, beta_u_k_star, beta_l_k_star,
            rho_k_star, A_k, b_k, Sigma_k, d_bar_k, c, r, q):
...
def Q_G(I, J, L, M_k, A_k, b_k, Sigma_k, d_bar_k, d_uk, d_lk, sigma_k,
        nu_hat, lambda_hat, q, r, y):
    ...
    opti_zeta_temp = Q.getAttr("X", zeta)
    opti_zeta = opti_zeta_temp.select("*")
    
    opti_eta_temp = Q.getAttr("X", eta)
    opti_eta = opti_eta_temp.select("*")

    zeta_temp = np.zeros(L)
    for l in range(L):
        zeta_temp[l] = opti_zeta_temp[l]

    eta_temp = np.zeros((J, L))
    for j in range(J):
        for l in range(L):
            eta_temp[j, l] = opti_eta_temp[j, l]

    G_k = np.ones(I)
    for i in range(I):
        G_k[i] = -1 * q[i] * np.dot(lambda_hat[i, :], opti_zeta)

    return Q_k, G_k, run_time_Q, zeta_temp, eta_temp


def fix_y_RMP(y_star_decompose, I, J, K, L, M_k, A_k, b_k, Sigma_k, d_bar_k,
              d_uk, d_lk, sigma_k, nu_hat, lambda_hat, f, p, q, r):
...
def decompose_RMP(I, J, K, L, M_k, A_k, b_k, Sigma_k, d_bar_k, d_uk, d_lk, sigma_k,
                  nu_hat, lambda_hat, f, p, q, r):
    
    time_begin = time.perf_counter()
    
    print("*" * 60)
    print("Decomposition Relaxed Master Problem start")
    ...
    while (flag == True):
        zeta_k = np.zeros((L, K))
        eta_k = np.zeros((J, L, K))
        for k in range(K):
            Q_all[k, cycle_index], G_all[:, k, cycle_index], \
                run_time_Q_all[cycle_index, k], zeta, eta \
                = Q_G(I, J, L, M_k,
                      A_k[:, :, k], b_k[:, k],
                      Sigma_k[:, :, k], d_bar_k[:, k],
        ...
        UB_tmp = np.dot(f.T, y_all[:, cycle_index]) + np.dot(p, Q_all[:, cycle_index])
        ...
        if (UB_all[cycle_index] <= (LB_all[cycle_index] + 0.001)):
    ...
    obj_val, alpha_star, beta_u_star, beta_l_star, rho_star, \
        run_time_FYRMP = fix_y_RMP(y_star_dec_temp, I, J, K, L, M_k, A_k, b_k, Sigma_k, d_bar_k,
                                   d_uk, d_lk, sigma_k, nu_hat, lambda_hat, f, p, q, r)
        
    run_time_dec_RMP = sum(run_time_Q_all) + sum(sum(run_time_LB_all)) \
        + run_time_FYRMP
        
    time_end = time.perf_counter()
    
    print("Decomposition Relaxed Master Problem end")
    print("optimal solution: {}".format(y_star_dec_temp))
    print("optimal value: {}".format(obj_val))
    print("computational time: {}".format(run_time_dec_RMP))
    print("*" * 60)

    return obj_val, y_star_decompose, alpha_star, beta_u_star, beta_l_star, rho_star, \
            run_time_dec_RMP, cycle_index, zeta_k, eta_k
...
def CLP_EW_EXACT(I, J, K, M_k, A_k, b_k, Sigma_k, d_bar_k, d_uk, d_lk, sigma_k,
                 c, f, p, q, r):
    ...
    gap_all_UB_minus_LB = np.ones(1)
    gap_all_LB_minus_UB = np.ones(1)
    ...
    while (flag == 1):
        ...
        print()
        print("=" * 60)
        print("              I_{}_J_{}_K_{}:the {}th iteration".format(I, J, K, times))

        obj_val_dec, y_star_dec, alpha_star_dec, beta_u_star_dec, beta_l_star_dec, rho_star_dec, \
        run_time_dec_RMP, cycle_index, zeta_k, eta_k\
            = decompose_RMP(I, J, K, L, M_k, A_k, b_k, Sigma_k, d_bar_k, d_uk, d_lk, sigma_k, nu_hat, lambda_hat,
                            f, p, q, r)
        ...
        if times == 1:
            ...
            LB = np.column_stack((LB, obj_val_dec))
        ...
        for k in range(K):
            ...
            Omega_k, nu_k_hat, lambda_k_hat, run_time_MICP[k], d_k_hat \
                = SP_MICP(I, J, K, M_k, y_star, beta_u_k_star, beta_l_k_star, rho_k_star,
                          A_k[:, :, k], b_k[:, k], Sigma_k[:, :, k], d_bar_k[:, k],
                          c, r, q)
            Omega[k] = Omega_k
        ...
        temp_UB = sum(f[i] * y_star[i] for i in range(I)) \
        ...
        flag_count_all = np.column_stack((flag_count_all, flag_count))


        alpha_star_all = np.column_stack((alpha_star_all, np.array(alpha_star_dec.select("*"))))
        ...
        run_time_dec_RMP_all = np.row_stack((run_time_dec_RMP_all, run_time_dec_RMP))


        run_time_MICP_all = np.row_stack((run_time_MICP_all, sum(run_time_MICP)))


        runtime_QLP_all = np.row_stack((runtime_QLP_all, sum(run_time_QLP)))


        time_loop_all = np.row_stack((time_loop_all, time_end_inside - time_begin_inside))
        ...
        gap_all_UB_minus_LB = np.row_stack((gap_all_UB_minus_LB, (UB_temp - LB_temp) / LB_temp))
        gap_all_LB_minus_UB = np.row_stack((gap_all_LB_minus_UB, (UB_temp - LB_temp) / UB_temp))
        ...
        print("-" * 60)
        print("                        flag_count                        ")
        print("                            {}".format(flag_count))
        print("-" * 60)
        print('                UB_ALL = {}'.format(UB_temp))
        print("-" * 60)
        print('                LB_ALL = {}'.format(LB_temp))

        print("-" * 60)
        print('              the {}th computional time: {}'.format(times,
                                                  run_time_dec_RMP + sum(run_time_MICP) + sum(run_time_QLP)))

        print('              the {}th gap = {}'.format(times,
                                                  np.round(gap_all_LB_minus_UB[times], 4)))
    ...
    run_time = sum(run_time_dec_RMP_all) + sum(run_time_MICP_all) + sum(runtime_QLP_all)

    return obj_val_dec, y_star, zeta_k, eta_k


def SAA(I, J, N, demand, q, c, r, f):
    m = gp.Model('SAA')
    y = m.addVars(I, vtype=GRB.BINARY, name='y')
    x = m.addVars(I, J, N, lb=0, ub=+GRB.INFINITY, vtype=GRB.CONTINUOUS, name='x')

    # c1
    for n in range(N):
        for j in range(J):
            m.addConstr(x.sum('*', j, n) - demand[j, n] <= 0)

    # c2
    for n in range(N):
        for i in range(I):
            m.addConstr(x.sum(i, "*", n) - q[i] * y[i] <= 0)
    #obj
    obj = gp.LinExpr()
    for n in range(N):
        term_x = gp.LinExpr()
        for i in range(I):
            term_x.add(gp.LinExpr(c[i, :] - r[:], x.select(i, "*", n)), 1)
        obj.add(term_x, 1/N)
        obj.addConstant(1/N * np.dot(r, demand[:, n]))
    obj.add(gp.LinExpr(f, y.select("*")), 1)
    ...
    for i in range(I):
        y_array[i] = y_val[i]
        if y_array[i] == 1:
            list_y.append('x_%d' % (i+1))
    x_array = np.zeros((I, J, N))
    for i in range(I):
        for j in range(J):
            for n in range(N):
                x_array[i, j, n] = x_val[i, j, n]
    TS = 0
    for n in range(N):
        for i in range(I):
            for j in range(J):
                TS += c[i, j] * x_array[i, j, n] * 1/N

    ST = 0
    for n in range(N):
        for j in range(J):
            ST += r[j] * (demand[j, n] - np.sum(x_array[:, j, n])) * 1/N
    return obj_val, y_array, TS, ST
...
def CFLP(y, d, f, I, J, c, r, q):
    ...
    return objValue, TSC, STC, solx_a